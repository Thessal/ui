...
use didius::logger::{Logger, config::{LoggerConfig, LogDestinationInfo}};
use std::sync::mpsc;
...
fn main() -> Result<()> {
    ...
    let first = &list[0];
    let symbol = first["pdno"].as_str().unwrap_or("101V3000").to_string(); // 'pdno' is symbol
    ...
    thread::spawn(move || {
        loop {
            ...
            for (id, o) in &orders {
                 summary.push_str(&format!("  [{}] {:?} {} @ {:?} (State: {:?}, filled: {}) Strategy: {:?}\n", 
    ...
    loop {
        ...
        match cmd {
            "b" | "s" => {
                // Get current price
                let best_ask = engine.get_order_book(&symbol).and_then(|b| b.get_best_ask().map(|(p,_)| p)).unwrap_or(Decimal::ZERO);
                let best_bid = engine.get_order_book(&symbol).and_then(|b| b.get_best_bid().map(|(p,_)| p)).unwrap_or(Decimal::ZERO);
                ...
                let tick_size = Decimal::new(5, 2); // 0.05 assumption? Or 1? Night future usually 0.05
                // Assuming tick size 0.05
                ...
                if cmd == "b" {
                    ...
                    println!("Placing Chain Order: Buy @ {}, Trigger @ {}, Chain Buy @ {}", buy_price, trigger_price, chain_price);
                    ...
                    let chain_price = best_bid - (tick_size * Decimal::from(5));
                     ...
                     println!("Placing Chain Order: Sell @ {}, Trigger @ {}, Chain Sell @ {}", sell_price, trigger_price, chain_price);
