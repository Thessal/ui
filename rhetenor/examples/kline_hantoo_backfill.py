...
from rhetenor.data import download_master, HantooClient
...
def main():
    ...
    if market.lower() != "kospi":
        raise NotImplementedError("Only KOSPI is supported for KOSPI50 filtering.")
    ...
    for d in target_dates:
        ...
        def safe_fetch(sym):
            for i in range(MAX_RETRIES):
                try:
                    ...
                    if i == MAX_RETRIES - 1:
                        print(f"Failed to fetch {sym} after {MAX_RETRIES} attempts: {e}")
        ...
        for idx, sym in enumerate(symbols):
            ...
            # Progress
            print(f"{d_str} [{idx+1}/{total_syms}] Fetching {sym}...", end='\r')
