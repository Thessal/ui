...
def download_master(market: str = "kospi", date_str: Optional[str] = None, 
    ...
    try:
        if market == 'kospi':
            ...
            raise ValueError(f"Invalid market: {market}. Must be 'kospi', 'kosdaq', or 'konex'.")
    except ImportError as e:
         raise ImportError(f"Spec for {market} not found or error importing: {e}")
    ...
    if loader:
        ...
        if data:
            if verbose:
                print(f"Loaded master data from S3: {loader.prefix}/{date_str}_{market}.json")
            return data
    ...
    try:
        ...
        with zipfile.ZipFile(io.BytesIO(response.content)) as zf:
            if mst_filename not in zf.namelist():
                raise FileNotFoundError(f"{mst_filename} not found in the downloaded zip file.")
        ...
        if verbose:
            print(f"Parsed {len(master_dict)} entries from {market} master file.")
...
class HantooClient:
    ...
    def inquire_time_itemchartprice(self, market_code: str, symbol: str, time_hhmmss: str, include_past: str = "N", etc_code: str = "", tr_cont: str = "") -> tuple[Dict[str, str], Dict[str, Any]]:
        ...
        while True:
            ...
            data = resp.json()
            if data['rt_cd'] != '0':
                print(f"API Error in item chart price fetch: {data.get('msg1')}")
                if all_output2:
                     last_data['output2'] = all_output2
                     return resp.headers, last_data
...
class HantooKlineLogger:
    ...
    def fetch_and_update(self):
        ...
        with concurrent.futures.ThreadPoolExecutor(max_workers=10) as executor:
            ...
            future_to_sym = {executor.submit(fetch, sym, self.exchange_code): sym for sym in self.symbols}
        ...
        for sym, rows in results.items():
            for row in rows:
                ...
                t_str = f"{self.target_date.strftime('%Y-%m-%d')}_{r_time[:4]}" # Minute resolution
        ...
        updates = self.wrapper.reconcile(time_aggregated, exchange_code=self.exchange_code)

        if updates:
            self.updates.extend(updates)
            if len(self.updates) >= 15:
                print(f"Uploading {len(self.updates)} records to S3...")
                self.wrapper.put(self.updates, exchange_code=self.exchange_code)
                self.updates = []
        else:
