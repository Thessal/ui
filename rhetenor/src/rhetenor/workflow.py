...
class Transpile(WorkFlow):
    def __init__(self, config_path, metadata_path):
        ...
        self.transpiler = morpho.Transpiler(config=config, prompt_path="prompt.json")
    ...
    def run(self):
        ...
        for hash in all_docs_hash:
            ...
            generation_result = self.transpiler.generate(system_context_args=dict(), user_prompt_args={"idea_text":idea_text})
    ...
    def save_result(generation_result, orig_metadata):
        ...
        for i, strat in enumerate(generation_result.strategies):
            ...
            # }
            output_data = "\n".join(["#"+strat.name, "#"+strat.description, strat.code])
            orig_metadata_path = ""
            output_metadata = generate_code_metadata(path="TODO", agent="machine#FFFFFF", related_path=orig_metadata_path)
